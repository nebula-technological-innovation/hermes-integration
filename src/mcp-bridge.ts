...
// MCP Server types
export type MCPServerType = 'memory' | 'playwright' | 'rag' | 'filesystem' | 'database';
...
export class MCPBridge {
  private servers: Map<MCPServerType, MCPConfig> = new Map();
  private defaultModel: AetherModel = 'orion';
  private defaultProvider: LLMProvider = 'aether';
  private defaultApiUrl = 'http://localhost:4011';
  ...
  private mockMemoryTool(tool: string, args: Record<string, any>): MCPToolResult {
  ...
  private mockPlaywrightTool(tool: string, args: Record<string, any>): MCPToolResult {
  ...
  private mockRAGTool(tool: string, args: Record<string, any>): MCPToolResult {
    if (tool === 'rag_search') {
      return {
        success: true,
        result: {
   ...
   */
  listTools(server: MCPServerType): MCPTool[] {
    const tools: Record<MCPServerType, MCPTool[]> = {
      memory: [
        {
          name: 'memory_store',
   ...
   */
  setServerEnabled(server: MCPServerType, enabled: boolean): void {
    const config = this.servers.get(server);
    if (config) {
   ...
   * Get server status
   */
  getStatus(): Record<MCPServerType, boolean> {
    const status: Record<MCPServerType, boolean> = {} as Record<MCPServerType, boolean>;
...
// Default MCP servers
export const DEFAULT_MCP_SERVERS: MCPConfig[] = [
